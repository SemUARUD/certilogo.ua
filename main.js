...
if (isIphone) {
    ...
    const gif = document.getElementById('ionphe-background-image');
    const png = document.getElementById('ionphe-background-image1');

    video.style.display = 'none';
    gif.style.display = 'block';

    videoContainer.style.backgroundImage = `url(${gif.src})`;
    ...
    setTimeout(() => {
        ...
        videoContainer.style.backgroundImage = `url(${png.src})`;

        const content = document.querySelector('.content');
        content.style.opacity = '0'; // Початково робимо елемент невидимим
        
        setTimeout(() => {
            content.style.opacity = '1'; // Зробити контент видимим через певний час
        }, 1200); // Показувати контент після 3 секунд
    }, 15000);
} else {
    const video = document.getElementById('video-background');
    const gif = document.getElementById('ionphe-background-image');
    ...
    setTimeout(() => {
        content.style.opacity = '1'; // Зробити контент видимим через певний час
    }, 5300); // Показувати контент після 3 секунд
}
